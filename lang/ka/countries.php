<?php

// lang/ka/countries.php

return [
    "AW" => "არუბა",
    "AF" => "ავღანეთი",
    "AO" => "ანგოლა",
    "AI" => "ანგილა",
    "AX" => "ალანდის კუნძულები",
    "AL" => "ალბანეთი",
    "AD" => "ანდორა",
    "AE" => "არაბთა გაერთიანებული საამიროები",
    "AR" => "არგენტინა",
    "AM" => "სომხეთი",
    "AS" => "ამერიკული სამოა",
    "AQ" => "ანტარქტიკა",
    "TF" => "ფრანგული სამხრეთ ტერიტორიები",
    "AG" => "ანტიგუა და ბარბუდა",
    "AU" => "ავსტრალია",
    "AT" => "ავსტრია",
    "AZ" => "აზერბაიჯანი",
    "BI" => "ბურუნდი",
    "BE" => "ბელგია",
    "BJ" => "ბენინი",
    "BQ" => "ბონერი, სენტ ევსტატუსი და საბა",
    "BF" => "ბურკინა-ფასო",
    "BD" => "ბანგლადეში",
    "BG" => "ბულგარეთი",
    "BH" => "ბაჰრეინი",
    "BS" => "ბაჰამის კუნძულები",
    "BA" => "ბოსნია და ჰერცეგოვინა",
    "BL" => "სენ-ბართლემი",
    "BY" => "ბელარუსი",
    "BZ" => "ბელიზი",
    "BM" => "ბერმუდა",
    "BO" => "პლურინაციონალური ქვეყანა ბოლივია",
    "BR" => "ბრაზილია",
    "BB" => "ბარბადოსი",
    "BN" => "ბრუნეი დარუსალამი",
    "BT" => "ბუტანი",
    "BV" => "ბუვეტის კუნძული",
    "BW" => "ბოტსვანა",
    "CF" => "ცენტრალური აფრიკის რესპუბლიკა",
    "CA" => "კანადა",
    "CC" => "ქოქოსის კუნძულები",
    "CH" => "შვეიცარია",
    "CL" => "ჩილე",
    "CN" => "ჩინეთი",
    "CI" => "კოტ-დ'ივუარი",
    "CM" => "კამერუნი",
    "CD" => "კონგო, დემოკრატიული რესპუბლიკა",
    "CG" => "კონგო",
    "CK" => "კუკის კუნძულები",
    "CO" => "კოლუმბია",
    "KM" => "კომორის კუნძულების კავშირი",
    "CV" => "კაბო-ვერდე",
    "CR" => "კოსტა-რიკა",
    "CU" => "კუბა",
    "CW" => "კიურასაო",
    "CX" => "შობის კუნძული",
    "KY" => "კაიმანის კუნძულები",
    "CY" => "კვიპროსი",
    "CZ" => "ჩეხეთი",
    "DE" => "გერმანია",
    "DJ" => "ჯიბუტი",
    "DM" => "დომინიკა",
    "DK" => "დანია",
    "DO" => "დომინიკის რესპუბლიკა",
    "DZ" => "ალჟირი",
    "EC" => "ეკვადორი",
    "EG" => "ეგვიპტე",
    "ER" => "ერიტრეა",
    "EH" => "დასავლეთი საჰარა",
    "ES" => "ესპანეთი",
    "EE" => "ესტონეთი",
    "ET" => "ეთიოპია",
    "FI" => "ფინეთი",
    "FJ" => "ფიჯი",
    "FK" => "ფოლკლენდის კუნძულები",
    "FR" => "საფრანგეთი",
    "FO" => "ფაროს კუნძულები",
    "FM" => "მიკრონეზიის ფედერალური შტატები",
    "GA" => "გაბონი",
    "GB" => "გაერთიანებული სამეფო",
    "GE" => "საქართველო",
    "GG" => "გერნსი",
    "GH" => "განა",
    "GI" => "გიბრალტარი",
    "GN" => "გვინეა",
    "GP" => "გვადელუპა",
    "GM" => "გამბია",
    "GW" => "გვინეა-ბისაუ",
    "GQ" => "ეკვატორული გვინეა",
    "GR" => "საბერძნეთი",
    "GD" => "გრენადა",
    "GL" => "გრენლანდია",
    "GT" => "გვატემალა",
    "GF" => "საფრანგეთის გვიანა",
    "GU" => "გუამი",
    "GY" => "გაიანა",
    "HK" => "ჰონგ-კონგი",
    "HM" => "ჰერდის და მაკდონალდის კუნძულები",
    "HN" => "ჰონდურასი",
    "HR" => "ხორვატია",
    "HT" => "ჰაიტი",
    "HU" => "უნგრეთი",
    "ID" => "ინდონეზია",
    "IM" => "მენის კუნძული",
    "IN" => "ინდოეთი",
    "IO" => "ბრიტანეთის ინდოეთის ოკეანის ტერიტორია",
    "IE" => "ირლანდია",
    "IR" => "ირანის ისლამური რესპუბლიკა",
    "IQ" => "ერაყი",
    "IS" => "ისლანდია",
    "IL" => "ისრაელი",
    "IT" => "იტალია",
    "JM" => "იამაიკა",
    "JE" => "ჯერსი",
    "JO" => "იორდანია",
    "JP" => "იაპონია",
    "KZ" => "ყაზახეთი",
    "KE" => "კენია",
    "KG" => "ყირგიზეთი",
    "KH" => "კამბოჯა",
    "KI" => "კირიბატი",
    "KN" => "სენტ-კიტსი და ნევისი",
    "KR" => "კორეა, რესპუბლიკა",
    "KW" => "ქუვეითი",
    "LA" => "ლაოს სახალხო დემოკრატიული რესპუბლიკა",
    "LB" => "ლიბანი",
    "LR" => "ლიბერია",
    "LY" => "ლიბია",
    "LC" => "სენტ-ლუსია",
    "LI" => "ლიხტენშტეინი",
    "LK" => "შრი-ლანკა",
    "LS" => "ლესოთო",
    "LT" => "ლიტვა",
    "LU" => "ლუქსემბურგი",
    "LV" => "ლატვია",
    "MO" => "მაკაო",
    "MF" => "სენ-მარტინი (ფრანგული ნაწილი)",
    "MA" => "მაროკო",
    "MC" => "მონაკო",
    "MD" => "მოლდოვის რესპუბლიკა",
    "MG" => "მადაგასკარი",
    "MV" => "მალდივები",
    "MX" => "მექსიკა",
    "MH" => "მარშალის კუნძულები",
    "MK" => "ჩრდილოეთ მაკედონია",
    "ML" => "მალი",
    "MT" => "მალტა",
    "MM" => "მიანმარი",
    "ME" => "მონტენეგრო",
    "MN" => "მონღოლეთი",
    "MP" => "ჩრდილოეთ მარიანას კუნძულები",
    "MZ" => "მოზამბიკი",
    "MR" => "მავრიტანია",
    "MS" => "მონსერატი",
    "MQ" => "მარტინიკა",
    "MU" => "მავრიკი",
    "MW" => "მალავი",
    "MY" => "მალაიზია",
    "YT" => "მაიოტა",
    "NA" => "ნამიბია",
    "NC" => "ახალი კალედონია",
    "NE" => "ნიგერი",
    "NF" => "ნორფოლკი",
    "NG" => "ნიგერია",
    "NI" => "ნიკარაგუა",
    "NU" => "ნიუე",
    "NL" => "ნიდერლანდები",
    "NO" => "ნორვეგია",
    "NP" => "ნეპალი",
    "NR" => "ნაურუ",
    "NZ" => "ახალი ზელანდია",
    "OM" => "ომანი",
    "PK" => "პაკისტანი",
    "PA" => "პანამა",
    "PN" => "პიტკარინი",
    "PE" => "პერუ",
    "PH" => "ფილიპინები",
    "PW" => "პალაუ",
    "PG" => "პაპუა-ახალი გვინეა",
    "PL" => "პოლონეთი",
    "PR" => "პუერტო-რიკო",
    "KP" => "კორეის სახალხო-დემოკრატიული რესპუბლიკა",
    "PT" => "პორტუგალია",
    "PY" => "პარაგვაი",
    "PS" => "პალესტინის სახელმწიფო",
    "PF" => "საფრანგეთის პოლინეზია",
    "QA" => "კატარი",
    "RE" => "რეიუნიონი",
    "RO" => "რუმინეთი",
    "RU" => "რუსეთის ფედრაცია",
    "RW" => "რუანდა",
    "SA" => "საუდის არაბეთი",
    "SD" => "სუდანი",
    "SN" => "სენეგალი",
    "SG" => "სინგაპური",
    "GS" => "სამხრეთ ჯორჯია და სამხრეთ სენდვიჩის კუნძულები",
    "SH" => "სენტ ჰელენა, ასცენსიონი და ტრისტან და კუნჰა",
    "SJ" => "შპიცბერგენი და იან-მაიენი",
    "SB" => "სოლომონის კუნძულები",
    "SL" => "სიერა-ლეონე",
    "SV" => "სალვადორი",
    "SM" => "სან-მარინო",
    "SO" => "სომალი",
    "PM" => "სენ-პიერი და მიქელონი",
    "RS" => "სერბეთი",
    "SS" => "სამხრეთ სუდანი",
    "ST" => "საო-ტომე და პრინსიპი",
    "SR" => "სურინამი",
    "SK" => "სლოვაკეთი",
    "SI" => "სლოვენია",
    "SE" => "შვედეთი",
    "SZ" => "ესვატინი",
    "SX" => "სენ-მარტინი (დანიური ნაწილი)",
    "SC" => "სეიშელის კუნძულები",
    "SY" => "სირიის არაბული რესპუბლიკა",
    "TC" => "ტერქსისა და კაიკოსის კუნძულები",
    "TD" => "ჩადი",
    "TG" => "ტოგო",
    "TH" => "ტაილანდი",
    "TJ" => "ტაჯიკეთი",
    "TK" => "ტოკელაუ",
    "TM" => "თურქმენეთი",
    "TL" => "აღმოსავლეთი ტიმორი",
    "TO" => "ტონგა",
    "TT" => "ტრინიდადი და ტობაგო",
    "TN" => "ტუნისი",
    "TR" => "თურქეთი",
    "TV" => "ტუვალუ",
    "TW" => "ტაივანი, ჩინეთის პროვინცია",
    "TZ" => "ტანზანიის გაერთიანებული რესპუბლიკა",
    "UG" => "უგანდა",
    "UA" => "უკრაინა",
    "UM" => "აშშ-ის მიმდებარე მცირე კუნძულები",
    "UY" => "ურუგვაი",
    "US" => "ამერიკის შეერთებული შტატები",
    "UZ" => "უზბეკეთი",
    "VA" => "წმიდა საყდარი (ქალაქი-სახელმწიფო ვატიკანი)",
    "VC" => "სენტ-ვინსენტი და გრენადინები",
    "VE" => "ვენესუელის ბოლივარული რესპუბლიკა",
    "VG" => "ვირჯინის კუნძულები, ბრიტანეთის",
    "VI" => "ვირჯინის კუნძულები, ა.შ.შ.",
    "VN" => "ვიეტ-ნამი",
    "VU" => "ვანუატუ",
    "WF" => "უოლისი და ფუტუნა",
    "WS" => "სამოა",
    "YE" => "იემენი",
    "ZA" => "სამხრეთ აფრიკის რესპუბლიკა",
    "ZM" => "ზამბია",
    "ZW" => "ზიმბაბვე",
];
